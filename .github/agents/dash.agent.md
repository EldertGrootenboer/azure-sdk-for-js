...
1. **Get the diff** — read the changed files and their diffs to
   understand what was modified.
2. **Filter scope** — skip test-only files, generated code under
   `src/generated/`, documentation-only changes, and `snippets.spec.ts`
   files (these contain documentation snippets, not real tests).
3. **Apply the checklist** — review each changed file against the
   performance checklist below. Only flag issues introduced or worsened
   by the PR, not pre-existing problems.
4. **Report findings** — use the output format at the end of this
   document. If no performance issues are found, say so explicitly in
   one sentence.
...
- **Large new dependency** — flag new dependencies that add > 50 KB
  minified to the bundle. Check if a lighter alternative exists. Use
  `npm pack --dry-run` or check `node_modules/<pkg>` size when needed.
